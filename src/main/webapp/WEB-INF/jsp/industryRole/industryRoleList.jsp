...
<body class="gray-bg">
                            ...
                            <div class="example">
                            	<form id="search-form" class="form-inline" role="form">
								    <!-- <div class="form-group">
						    		    <label>姓名：</label>
						    		    <input type="text" name="industryRole.realName" class="form-control">
						  		    </div>
						  		    <div class="form-group">
						    		    <label>状态：</label>
						    		    <select name="industryRole.dataStatus" class="form-control">
                                ...
                                <table id="industryRoleListTable" 
                                data-toggle="table"
                                data-method="post"
                                data-url="${projectPath }/industryRole/industryRoleListData.action" 
                                ...
                                data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="roleName" data-align="center">行业角色名称</th>
                                            <th data-field="industryName" data-align="center">所属行业</th>
                                            <th data-field="totalStudyTime" data-align="center">总学时(秒)</th>
                                            <th data-field="standardTypeName" data-align="center">达标类型</th>
    ...
    var $industryRoleListTable = $("#industryRoleListTable");
    $(document).on('click','#add',function(){
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-industryRole"></i> 添加行业角色',
			area: ['800px', '600px'],
			content: '${projectPath}/industryRole/industryRoleAdd.action'
		});
	});
    $(document).on('click','#edit',function(){
    	var rows = $industryRoleListTable.bootstrapTable('getSelections');
		if(rows.length!=1){
		...
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-industryRole"></i> 编辑行业角色',
			area: ['800px', '600px'],
			content: '${projectPath}/industryRole/industryRoleEdit.action?id='+rows[0].id
		});
	});
    $(document).on('click','#del',function(){
		...
		layer.confirm('确定删除选中的行业角色吗？', function(index){
			/* layer.close(index);
			var ids = '';
			...
			$.ajax({
				type:"POST",
				url:'${projectPath}/industryRole/industryRoleDelete.action?id=' + rows[0].id,
				success:function(data){
					if(data.resultInfo.success){
						$industryRoleListTable.bootstrapTable("refresh");
