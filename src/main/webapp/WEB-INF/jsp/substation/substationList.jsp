...
<body class="gray-bg">
                            ...
                            <div class="example">
                            	<form id="search-form" class="form-inline" role="form">
								    <!-- <div class="form-group">
						    		    <label>姓名：</label>
						    		    <input type="text" name="substation.realName" class="form-control">
						  		    </div>
						  		    <div class="form-group">
						    		    <label>状态：</label>
						    		    <select name="substation.dataStatus" class="form-control">
                                ...
                                <table id="substationListTable" 
                                data-toggle="table"
                                data-method="post"
                                data-url="${projectPath }/substation/substationListData.action" 
                                ...
                                data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="address" data-align="center">分局地址</th>
    ...
    var $substationListTable = $("#substationListTable");
    $(document).on('click','#add',function(){
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-substation"></i> 添加分局',
			area: ['800px', '300px'],
			content: '${projectPath}/substation/substationAdd.action'
		});
	});
    $(document).on('click','#edit',function(){
    	var rows = $substationListTable.bootstrapTable('getSelections');
		if(rows.length!=1){
		...
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-substation"></i> 编辑分局',
			area: ['800px', '300px'],
			content: '${projectPath}/substation/substationEdit.action?id='+rows[0].id
		});
	});
    $(document).on('click','#del',function(){
		...
		layer.confirm('确定删除选中的分局？', function(index){
			...
			$.ajax({
				type:"POST",
				url:'${projectPath}/substation/substationDelete.action?id=' + rows[0].id,
				success:function(data){
					if(data.resultInfo.success){
						$substationListTable.bootstrapTable("refresh");
