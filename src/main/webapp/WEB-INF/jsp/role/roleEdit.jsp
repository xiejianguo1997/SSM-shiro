...
<body>
    <div class="wrapper wrapper-content animated fadeInRight">
        ...
        <form class="form-horizontal m-t" id="roleForm" action="${projectPath }/role/roleEdit.action" method="post">
        	<input type="hidden" name="id" value="${role.id }">
            <div class="form-group">
                <label class="col-sm-3 control-label">角色名称：</label>
                <div class="col-sm-8">
                    <input id="roleName" name="roleName" class="form-control" type="text" required="required" value="${role.roleName }">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">状态：</label>
                <div class="col-sm-8">
                    <label class="radio-inline i-checks"><input type="radio"  name="dataStatus" value="1" required="required" <c:if test="${role.dataStatus==1 }">checked="checked"</c:if>>启用</label>
                    <label class="radio-inline i-checks"><input type="radio" name="dataStatus" value="0" <c:if test="${role.dataStatus==0 }">checked="checked"</c:if>>禁用</label>
                </div>
            </div>
            <div class="form-group">
