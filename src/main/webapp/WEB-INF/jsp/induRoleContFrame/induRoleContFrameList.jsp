...
<body class="gray-bg">
                            ...
                            <div class="example">
                            	<form id="search-form" class="form-inline" role="form">
								    <!-- <div class="form-group">
						    		    <label>姓名：</label>
						    		    <input type="text" name="induRoleContFrame.realName" class="form-control">
						  		    </div>
						  		    <div class="form-group">
						    		    <label>状态：</label>
						    		    <select name="induRoleContFrame.dataStatus" class="form-control">
                                ...
                                <table id="induRoleContFrameListTable" 
                                data-toggle="table"
                                data-method="post"
                                data-url="${projectPath }/induRoleContFrame/induRoleContFrameListData.action" 
                                ...
                                data-mobile-responsive="true">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-field="contentFrameworkName" data-align="center">内容权限</th>
    ...
    var $induRoleContFrameListTable = $("#induRoleContFrameListTable");
    $(document).on('click','#add',function(){
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-induRoleContFrame"></i> 添加行业角色和内容架构',
			area: ['800px', '300px'],
			content: '${projectPath}/induRoleContFrame/induRoleContFrameAdd.action'
		});
	});
    $(document).on('click','#edit',function(){
    	var rows = $induRoleContFrameListTable.bootstrapTable('getSelections');
		if(rows.length!=1){
		...
		layer.open({
			type: 2, 
			title:'<i class="glyphicon glyphicon-induRoleContFrame"></i> 编辑行业角色和内容架构',
			area: ['800px', '300px'],
			content: '${projectPath}/induRoleContFrame/induRoleContFrameEdit.action?id='+rows[0].id
		});
	});
    $(document).on('click','#del',function(){
		...
		layer.confirm('确定删除选中的行业角色和内容架构？', function(index){
			/* layer.close(index);
			var ids = '';
			...
			$.ajax({
				type:"POST",
				url:'${projectPath}/induRoleContFrame/induRoleContFrameDelete.action?id=' + rows[0].id,
				success:function(data){
					if(data.resultInfo.success){
						$induRoleContFrameListTable.bootstrapTable("refresh");
